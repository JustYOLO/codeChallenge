...
arr = [list(map(int, stdin.readline().split())) for i in range(6)]
w = 0; w_idx = 0
h = 0; h_idx = 0
for i in range(6):
    if arr[i][0] == 1 or arr[i][0] == 2:
        ...
        if h < arr[i][1]:
            h = arr[i][1]
            h_idx = i
smallW = abs(arr[(w_idx - 1) % 6][1] - arr[(w_idx + 1) % 6][1])
smallH = abs(arr[(h_idx - 1) % 6][1] - arr[(h_idx + 1) % 6][1])
print(((w*h) - (smallW*smallH))* k)